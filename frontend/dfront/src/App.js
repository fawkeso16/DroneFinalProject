...
function App() {
  ...
  const [noFlyNodes, setNoFlyNodes] = useState([]);
  ...
  const getNoFlyZone = async () => {
    try {
      const response = await fetch("http://localhost:8080/api/map/noFlyZone");

      if (!response.ok) {
        setStatusMessage("Failed to fetch no-fly zone");
        return;
      }

      const data = await response.json();
      if (Array.isArray(data)) {
        setNoFlyNodes(data.filter(n => n && n.x != null && n.y != null));
      }

      setStatusMessage("No-fly zone fetched");
      setTimeout(() => setStatusMessage(""), 3000);
    } catch (err) {
      console.error("Failed to update destination:", err);
    }
...
return (
  <div>
    ...
    <div className="map-toolbar">
      ...
      <button className='btn btn--primary' onClick={() => setShowNoFlyZone(prev => !prev)}>
        {showNoFlyZone ? "Hide No-Fly Zone" : "Show No-Fly Zone"}
      </button>
